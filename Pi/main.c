...
//  main.c
//  Pi
//  A program to find pi to a specified accuracy using the Gregory-Leibniz converging series.
//  Created by Jeremy Schmidt on 4/5/13.
//  Copyright (c) 2013 Jeremy Schmidt. Some rights reserved.
//


// Yay for importing and declaring variables!
...
long maxIterations = 2428702234; // At this many iterations, the memory limit of a double is reached. Nothing to be done after that.


// Primary function to carry out the converging formula

double findPi (long unsigned iterations, double pi, long unsigned int denominator)
{
    double newPi = 0;
    
    if (! (iterations % 2) || iterations == 0)
    {
        newPi = pi - ((double)4 / (double)denominator);
    }
    else
    {
        newPi = pi + ((double)4 / (double)denominator);
...
/* Reducing the numbers is nessesary for determining if I have true pi.  Because the convergance fluctuates back and forth around pi with the fluctuation becoming smaller, if two consective results are the same to a given decemal place, then up to that point is true pi.  Reducing it allows me to limit the number to just the part I want to compare.
 */
...
int main ()
{
    ...
    while (iterations < maxIterations)
    {
        ...
        if (solid)
        {
            ...
            printf("%.14f found in %li iterations    (%f seconds.)\n", solid, iterations, ((float)successTime)/CLOCKS_PER_SEC);
            placeToCompare++;
            noSuccess = 0;
        }
        else
